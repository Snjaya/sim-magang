        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="max-w-xl text-gray-900 dark:text-gray-100">
                    ...
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ $task->deskripsi }}</p>
                    <p class="mt-4 text-sm"><strong>Dikerjakan oleh:</strong>
                        @foreach ($task->users as $peserta)
            ...
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="max-w-xl text-gray-900 dark:text-gray-100">
                    ...
                    @if ($task->status == 'verifikasi')
                        ...
                        @elseif($task->jenis == 'lapangan_teknis')
                            ...
                            <div
                                ...
                                {{ $task->laporan_deskripsi }}</div>
                        @endif

                        {{-- FORM AKSI VERIFIKASI --}}
                        <form method="POST" action="{{ route('pembimbing.task.verify', $task->id) }}"
                    ...
                    @else
                        ...
                        <p class="mt-1">Status saat ini: <strong>{{ ucfirst($task->status) }}</strong></p>
                    @endif
                </div>
            </div>
